<template>
    <div class="submit-container">
      <div class="form-box">
        ...
        <div v-if="mode === 'batch'" class="common-settings">
          <h3>🛠️ 公共设置 (应用于所有)</h3>
          <p class="hint-text">填写一次，应用到下面所有商品。之后可以单独修改。</p>
  ...
  // 批量状态
  const batchList = ref([]) // { file, preview, name, price, character }
  ...
  const handleBatchFiles = (e) => {
    ...
    files.forEach(file => {
      ...
      batchList.value.push({
        ...
        name: defaultName,
        price: null,
        character: form.character // 继承公共设置
      })
    })
  }
  ...
  const uploadFile = async (file) => {
    ...
    const fileName = `batch-${Date.now()}-${Math.random().toString(36).slice(2)}.${fileExt}`
  ...
  <style scoped>
  .submit-container { padding: 40px 20px; background: #f0f9f9; min-height: 100vh; display: flex; justify-content: center; font-family: sans-serif; }
  .form-box { background: white; width: 100%; max-width: 700px; padding: 30px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); }
