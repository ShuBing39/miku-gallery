        ...
        <div class="form-group identity-section" v-if="myCircles.length > 0">
          ...
          <div class="identity-selector">
            ...
            <p class="hint" v-if="form.identity !== 'individual'">
              * 该企划将显示在 [{{ getCircleName(form.identity) }}] 的社团主页。
            </p>
          </div>
        </div>
...
const fetchMyCircles = async () => {
  try {
    ...
    if (!userId) return
    const { data } = await supabase.from('circle_members').select('circles(id, name)').eq('user_id', userId)
    if (data) myCircles.value = data.map(item => item.circles).filter(Boolean)
  } catch (e) { console.error(e) }
}

const getCircleName = (id) => myCircles.value.find(i => i.id === id)?.name || ''
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      start_date: form.start_date || new Date().toISOString().split('T')[0], // 默认今天
    ...
    // 1. 提交并获取返回的 project 对象
    const newProject = await submitProject(payload)
    
    alert('✅ 企划发布成功！正在跳转详情页...')
