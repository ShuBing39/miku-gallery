  ...
  <div class="project-detail-container">
    ...
    <div v-else-if="project" class="project-content">
      ...
      <div class="tab-content-area">
        <div v-if="currentTab === 'overview'" class="tab-pane">
          ...
          <div class="gallery-section"><h3>🎨 创作展示 / 设定集</h3><div v-if="project.gallery_urls && project.gallery_urls.length" class="gallery-grid"><div v-for="(img, idx) in project.gallery_urls" :key="idx" class="gallery-item" @click="viewImage(img)"><img :src="img" loading="lazy"></div></div><div v-else class="empty-text">暂无展示图片，主催可在管理后台上传</div></div>
        </div>
        <div v-if="currentTab === 'timeline'" class="tab-pane">
          <div v-if="!canView('timeline')" class="lock-screen">🔒 该区域仅对内部可见</div>
          <div v-else>
            <div v-if="isManager" class="mini-form"><input v-model="newTimeline.title" placeholder="输入里程碑 (如: PV发布)" class="std-input"><input type="date" v-model="newTimeline.date" class="std-input" title="不填默认今天"><button @click="addTimeline" class="btn-mini-save">添加节点</button></div>
            <div class="timeline-list"><div v-for="node in timeline" :key="node.id" class="t-node" :class="{ done: node.is_completed }"><div class="t-line"></div><div class="t-dot" @click="isManager && toggleTimeline(node)"></div><div class="t-info"><div class="t-head"><strong>{{ node.title }}</strong><span class="t-date">{{ formatDate(node.event_date) }}</span></div><div class="t-meta">操作人: {{ node.profiles?.username || '未知' }}</div><button v-if="isManager" @click="deleteTimeline(node.id)" class="del-text">删除</button></div></div><div v-if="timeline.length === 0" class="empty-mini">暂无进度节点</div></div>
          </div>
        </div>
        <div v-if="currentTab === 'tasks'" class="tab-pane">
          <div v-if="!canView('tasks')" class="lock-screen">🔒 该区域仅对内部可见</div>
          <div v-else>
            <div v-if="isManager" class="mini-form"><input v-model="newTask.content" placeholder="快速发布任务..." class="std-input grow"><button @click="showTaskModal = true" class="btn-mini-save">高级发布</button></div>
            <div class="task-grid-view"><div v-for="task in tasks" :key="task.id" class="task-box"><div class="task-status-bar" :class="task.status"></div><div class="task-main"><div class="task-head-row"><h4>{{ task.content }}</h4><span v-if="task.is_collaborative" class="tag-collab">👥 多人协作</span></div><div class="task-assignee"><div v-if="task.is_collaborative" class="multi-avatars"><span v-if="!task.claimants || task.claimants.length === 0" class="unassigned">⏳ 待认领 (可多人)</span><div v-else class="avatar-row"><span v-for="u in task.claimants" :key="u.id" class="mini-u" :title="u.username">🏃 {{ u.username }}</span></div></div><div v-else><span v-if="task.assignee?.username">🏃 {{ task.assignee.username }}</span><span v-else class="unassigned">⏳ 待认领 (单人)</span></div></div><div class="task-dates">{{ formatDate(task.start_at) }} - {{ formatDate(task.end_at) }}</div><div class="task-btns"><button v-if="canClaim(task)" @click="claimTask(task)" class="btn-claim">✋ 认领</button><button v-if="canReturn(task)" @click="returnTask(task)" class="btn-return">↩️ 退回</button><button v-if="isManager" @click="deleteTask(task.id)" class="btn-del">🗑️</button><button v-if="isManager || isTaskOwner(task)" @click="toggleTaskStatus(task)" class="btn-check">{{ task.status === 'done' ? '重做' : '完成' }}</button></div></div></div><div v-if="tasks.length === 0" class="empty-mini">暂无任务</div></div>
          </div>
        </div>
        <div v-if="currentTab === 'team'" class="tab-pane">
          <div v-if="!canView('team')" class="lock-screen">🔒 团队名单仅对内部可见</div>
          <div v-else class="team-list"><div class="team-header-row"><span>成员</span><span>身份/头衔</span><span>当前任务</span><span>加入时间</span><span>操作</span></div><div v-for="m in members" :key="m.id" class="team-row"><div class="m-name">{{ m.profiles?.username || '未知用户' }}</div><div class="m-role"><span class="role-tag" :class="getRoleClass(m.role)">{{ m.role || '成员' }}</span><button v-if="isManager" @click="openRoleModal(m)" class="btn-edit-role">✎</button></div><div class="m-tasks">{{ getMemberTaskCount(m.user_id) }} 个进行中</div><div class="m-join">{{ formatDate(m.joined_at || m.created_at) }}</div><div class="m-action"><button v-if="isManager && m.user_id !== currentUser.id" @click="kickMember(m)" class="btn-kick">移出</button></div></div></div>
        </div>
        <div v-if="currentTab === 'internal'" class="tab-pane chat-pane">
           <div v-if="!isMember" class="lock-screen">🔒 内部讨论区仅限成员查看</div>
           <template v-else><div class="chat-window" ref="internalChatRef"><div v-for="msg in internalComments" :key="msg.id"><div v-if="msg.type === 'system'" class="sys-msg-row"><span class="sys-pill">📢 {{ msg.content }} <small>{{ formatTime(msg.created_at) }}</small></span></div><div v-else class="chat-msg" :class="{ mine: msg.user_id === currentUser?.id }"><div class="msg-meta">{{ msg.profiles?.username || '成员' }} · {{ formatTime(msg.created_at) }}</div><div class="msg-bubble internal">{{ msg.content }}</div></div></div></div><div class="chat-input"><input v-model="newInternalMsg" placeholder="发布内部通知或讨论..." @keyup.enter="sendInternalMsg"><button @click="sendInternalMsg">发送</button></div></template>
        </div>
        <div v-if="currentTab === 'public'" class="tab-pane chat-pane">
           <div v-if="!canView('guest_comments')" class="lock-screen">🔒 留言区已关闭</div>
           <template v-else><div class="chat-window" ref="publicChatRef"><div v-if="publicComments.length === 0" class="empty-text">暂时没有游客留言</div><div v-for="msg in publicComments" :key="msg.id" class="chat-msg public"><div class="msg-meta">{{ msg.profiles?.username || '路人' }} · {{ formatTime(msg.created_at) }}</div><div class="msg-bubble">{{ msg.content }}</div><button v-if="isManager" @click="deleteComment(msg.id)" class="del-cmt">×</button></div></div><div class="chat-input"><input v-model="newPublicMsg" placeholder="支持一下..." @keyup.enter="sendPublicMsg"><button @click="sendPublicMsg">留言</button></div></template>
        </div>
      </div>
    </div>
      ...
      <div class="manager-console">
        ...
        <div class="console-body">
          ...
          <section class="console-section">
            ...
            <div class="split-row">
              <div class="half-sec">
                <div class="upload-bar"><input type="file" multiple accept="image/*" @change="handleAlbumUpload" :disabled="uploadingAlbum"><span v-if="uploadingAlbum">上传中...</span></div>
                <div class="preview-grid-mini"><div v-for="(url, idx) in galleryUrls" :key="idx" class="pg-item"><img :src="url"><button @click="removeGalleryImage(idx)" class="btn-x">×</button></div></div>
              </div>
              <div class="half-sec">
                <button @click="generateInvite" class="btn-action-sm">✨ 生成邀请码</button>
                <div class="invite-table-box"><div v-for="inv in activeInvites" :key="inv.id" class="invite-row-mini"><span class="code-font">{{ inv.code }}</span><button @click="deleteInvite(inv.id)" class="btn-text-danger">关闭</button></div></div>
              </div>
            </div>
          </section>

          <section class="console-section finish-zone">
            <h3>🏁 完结与归档</h3>
            <p class="section-desc">当企划结束时，您可以选择仅修改状态，或将其作为完成品收录。</p>
            <div class="finish-actions">
              <button @click="finishProjectSimple" class="btn-finish-simple">🛑 仅结束企划</button>
              <button @click="showArchiveModal = true" class="btn-finish-archive">📚 完结并收录到维基</button>
            </div>
          </section>
          ...
          <section class="console-section danger-zone">
            ...
            <div class="danger-actions-row">
               <button @click="handleTransfer" class="btn-warning-lg">👑 移交主办权</button>
               <button @click="handleDelete" class="btn-danger-lg">💥 解散/删除企划</button>
            </div>
          </section>
    ...
    <div v-if="showArchiveModal" class="modal-overlay">
      <div class="modal-content">
        <h3>🎉 恭喜企划完结！</h3>
        <p>请确认收录到维基的信息：</p>
        <div class="form-group"><label>作品名称</label><input v-model="archiveForm.name" class="std-input"></div>
        <div class="form-group"><label>作品分类</label><select v-model="archiveForm.category" class="std-input"><option>同人制品</option><option>CD/专辑</option><option>画集/书籍</option><option>手办/模型</option></select></div>
        <div class="form-group"><label>发售/公开日期</label><input type="date" v-model="archiveForm.release_date" class="std-input"></div>
        <div class="form-group"><label>发售价 (可选)</label><input v-model="archiveForm.price" placeholder="例如: 50.00" class="std-input"></div>
        <div class="modal-actions"><button @click="showArchiveModal = false">取消</button><button @click="confirmArchive" class="confirm">🚀 确认收录并完结</button></div>
      </div>
    </div>
    ...
    <div v-if="showTaskModal" class="modal-overlay">
      <div class="modal-content">
        ...
        <div class="checkbox-group"><label><input type="checkbox" v-model="newTask.is_open_claim"> 开放认领</label><label class="collab-check"><input type="checkbox" v-model="newTask.is_collaborative"> 👥 允许多人协作</label></div>
...
const archiveForm = ref({ name: '', category: '同人制品', release_date: '', price: '' })
...
const initData = async () => {
  ...
  await fetchProject()
  if (project.value) {
    await Promise.all([fetchTimeline(), fetchTasks(), fetchMembers(), fetchComments('internal'), fetchComments('public')])
    archiveForm.value.name = project.value.name
    archiveForm.value.release_date = new Date().toISOString().split('T')[0]
  }
  loading.value = false
}

// ... (核心数据获取) ...
const fetchProject = async () => { const { data } = await supabase.from('projects').select('*').eq('id', projectId).single(); if (data) { let uploaderName = '未知'; if (data.uploader_id) { const { data: u } = await supabase.from('profiles').select('username').eq('id', data.uploader_id).single(); if (u) uploaderName = u.username }; project.value = { ...data, uploader_name: uploaderName }; editForm.value = { ...data }; galleryUrls.value = data.gallery_urls || []; linkJson.value = JSON.stringify(data.external_links || []); if (data.visibility_settings) visibilityForm.value = { ...visibilityForm.value, ...data.visibility_settings } } }
const mapProfiles = async (items, idField, targetField = 'profiles') => { if (!items || items.length === 0) return []; const ids = [...new Set(items.map(i => i[idField]).filter(Boolean))]; if (ids.length === 0) return items; const { data: profiles } = await supabase.from('profiles').select('id, username').in('id', ids); return items.map(item => { const p = profiles?.find(p => p.id === item[idField]); return { ...item, [targetField]: p || { username: '未知' } } }) }
...
const fetchTasks = async () => { const { data: taskData } = await supabase.from('project_tasks_v2').select('*').eq('project_id', projectId).order('created_at'); if (!taskData) { tasks.value = []; return }; let tasksWithSingle = await mapProfiles(taskData, 'assignee_id', 'assignee'); const multiTaskIds = tasksWithSingle.filter(t => t.is_collaborative).map(t => t.id); if (multiTaskIds.length > 0) { const { data: claims } = await supabase.from('project_task_claims').select('*').in('task_id', multiTaskIds); const claimUserIds = [...new Set(claims?.map(c => c.user_id) || [])]; let userMap = {}; if (claimUserIds.length > 0) { const { data: profiles } = await supabase.from('profiles').select('id, username').in('id', claimUserIds); profiles?.forEach(p => userMap[p.id] = p) }; tasksWithSingle = tasksWithSingle.map(t => { if (t.is_collaborative) { const myClaims = claims?.filter(c => c.task_id === t.id) || []; t.claimants = myClaims.map(c => userMap[c.user_id] || { username: '未知' }).filter(u => u) }; return t }) }; tasks.value = tasksWithSingle }
...
const fetchComments = async (type) => { let query = supabase.from('project_comments').select('*').eq('project_id', projectId).order('created_at'); if (type === 'internal') query = query.in('type', ['internal', 'system']); else query = query.eq('type', type); const { data } = await query; const mapped = await mapProfiles(data, 'user_id', 'profiles'); if (type === 'internal') internalComments.value = mapped; else publicComments.value = mapped }
...
const handleTransfer = async () => {
  const name = prompt('👑 移交主办权\n\n请输入要移交的成员用户名：')
  if (!name) return
  const target = members.value.find(m => m.profiles?.username === name)
  if (!target) return alert('❌ 找不到该成员，请确保输入完全正确')
  
  if (confirm(`⚠️ 确认将主催权限移交给 [${target.profiles.username}]？\n\n移交后，您的身份将变为普通成员。`)) {
     const { error } = await supabase.from('projects').update({ uploader_id: target.user_id }).eq('id', projectId)
     if (!error) {
       await supabase.from('project_members').update({ role: '主催' }).eq('user_id', target.user_id).eq('project_id', projectId)
       await supabase.from('project_members').update({ role: '协力' }).eq('user_id', currentUser.value.id).eq('project_id', projectId)
       await logSystemAction('transfer', `主催权限已移交给 ${target.profiles.username}`)
       alert('移交成功，您已不再是主催。'); window.location.reload()
     } else { alert('移交失败: ' + error.message) }
  }
}

const handleDelete = async () => {
  const confirmName = prompt(`💥 解散企划确认\n\n此操作将永久删除企划 [${project.value.name}] 及其所有任务、进度和留言。\n\n请输入企划名称以确认删除：`)
  if (confirmName === project.value.name) {
    const { error } = await supabase.from('projects').delete().eq('id', projectId)
    if (error) return alert('删除失败: ' + error.message)
    alert('企划已解散'); router.push('/projects')
  } else {
    if (confirmName !== null) alert('名称输入不匹配，已取消操作')
  }
}

// ... (其他操作逻辑保持不变) ...
const finishProjectSimple = async () => { if (!confirm('确定结束此企划吗？')) return; await supabase.from('projects').update({ recruit_status: 'ended' }).eq('id', projectId); await logSystemAction('finish', `${getUserName()} 将企划标记为已完结`); editForm.value.recruit_status = 'ended'; alert('已结束') }
const confirmArchive = async () => { if (!archiveForm.value.name) return alert('请输入名称'); try { const { error } = await supabase.from('items').insert({ name: archiveForm.value.name, category: archiveForm.value.category, image_url: project.value.image_url, description: project.value.description, market_price: archiveForm.value.price || 0, release_date: archiveForm.value.release_date, status: 'approved', is_fan_work: true, uploader_id: currentUser.value.id }); if (error) throw error; await supabase.from('projects').update({ recruit_status: 'ended' }).eq('id', projectId); await logSystemAction('archive', `${getUserName()} 将企划收录到了维基`); alert('成功收录！'); showArchiveModal.value = false; showManagePanel.value = false; fetchProject() } catch (e) { alert('收录失败: ' + e.message) } }
const openManagePanel = () => { showManagePanel.value = true; fetchInvites() }
const handleAlbumUpload = async (event) => { const files = event.target.files; if (!files || files.length === 0) return; uploadingAlbum.value = true; let count = 0; try { for (const file of files) { const fileExt = file.name.split('.').pop(); const fileName = `album/${Date.now()}_${Math.random().toString(36).slice(2)}.${fileExt}`; await supabase.storage.from('user_uploads').upload(fileName, file); const { data: { publicUrl } } = supabase.storage.from('user_uploads').getPublicUrl(fileName); galleryUrls.value.push(publicUrl); count++ }; if (count > 0) await logSystemAction('gallery', `${getUserName()} 上传了 ${count} 张新图片`) } catch (e) { alert('上传失败') } finally { uploadingAlbum.value = false } }
const removeGalleryImage = (idx) => { galleryUrls.value.splice(idx, 1) }
const saveSettings = async () => { try { const links = JSON.parse(linkJson.value); await supabase.from('projects').update({ name: editForm.value.name, description: editForm.value.description, recruit_status: editForm.value.recruit_status, image_url: editForm.value.image_url, gallery_urls: galleryUrls.value, external_links: links, visibility_settings: visibilityForm.value }).eq('id', projectId); await logSystemAction('update_info', `${getUserName()} 更新了企划设置`); alert('保存成功'); showManagePanel.value = false; fetchProject() } catch (e) { alert('保存失败') } }
...
const addTask = async () => { const { error } = await supabase.from('project_tasks_v2').insert({ project_id: projectId, content: newTask.value.content, start_at: newTask.value.start_at || null, end_at: newTask.value.end_at || null, is_open_claim: newTask.value.is_open_claim, is_collaborative: newTask.value.is_collaborative, creator_id: currentUser.value.id }); if(error) alert('发布失败'); else { showTaskModal.value = false; await logSystemAction('task_create', `${getUserName()} 发布了新任务: ${newTask.value.content}`); newTask.value.content = ''; fetchTasks() } }
const claimTask = async (task) => { if(!confirm('认领?')) return; if (task.is_collaborative) { await supabase.from('project_task_claims').insert({ task_id: task.id, user_id: currentUser.value.id }) } else { await supabase.from('project_tasks_v2').update({ assignee_id: currentUser.value.id }).eq('id', task.id) }; await logSystemAction('task_claim', `${getUserName()} 认领了任务: ${task.content}`); setTimeout(() => { fetchTasks(); fetchMembers() }, 500) }
const returnTask = async (task) => { if(!confirm('退回?')) return; if (task.is_collaborative) { await supabase.from('project_task_claims').delete().eq('task_id', task.id).eq('user_id', currentUser.value.id) } else { await supabase.from('project_tasks_v2').update({ assignee_id: null }).eq('id', task.id) }; await logSystemAction('task_return', `${getUserName()} 退回了任务: ${task.content}`); setTimeout(() => { fetchTasks() }, 500) }
...
const sendInternalMsg = async () => { if(!newInternalMsg.value) return; const {error} = await supabase.from('project_comments').insert({ project_id: projectId, user_id: currentUser.value.id, content: newInternalMsg.value, type: 'internal' }); if(!error) { newInternalMsg.value = ''; fetchComments('internal') } }
const sendPublicMsg = async () => { if(!newPublicMsg.value) return; const {error} = await supabase.from('project_comments').insert({ project_id: projectId, user_id: currentUser.value.id, content: newPublicMsg.value, type: 'public' }); if(!error) { newPublicMsg.value = ''; fetchComments('public') } }
...
const isTaskOwner = (task) => { if (task.is_collaborative) return task.claimants?.some(u => u.id === currentUser.value.id); else return task.assignee_id === currentUser.value.id }
const canClaim = (task) => { if (!isMember.value) return false; if (!task.is_open_claim) return false; if (task.is_collaborative) { const alreadyClaimed = task.claimants?.some(u => u.id === currentUser.value.id); return !alreadyClaimed } else { return !task.assignee_id } }
const canReturn = (task) => { if (task.is_collaborative) return task.claimants?.some(u => u.id === currentUser.value.id); else return task.assignee_id === currentUser.value.id }
</script>
...
.finish-zone { background: #e8f5e9; border: 1px solid #c8e6c9; }
.finish-zone h3 { color: #2e7d32; border-left-color: #2e7d32; }
.finish-actions { display: flex; gap: 15px; margin-top: 15px; }
.btn-finish-simple { padding: 10px 20px; border: 1px solid #aaa; background: white; border-radius: 6px; cursor: pointer; }
.btn-finish-archive { padding: 10px 20px; border: none; background: #2e7d32; color: white; border-radius: 6px; cursor: pointer; font-weight: bold; }
...
.btn-close-console { background: none; border: 1px solid white; color: white; padding: 5px 15px; border-radius: 20px; cursor: pointer; }
