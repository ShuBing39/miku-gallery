<template>
      ...
      <template v-else-if="project">
        <div class="project-header">
          <div class="header-content">
            ...
            <div class="title-row">
              ...
              <span class="status-badge" :class="project.status">{{ project.status === 'approved' ? '进行中' : '审核中' }}</span>
            </div>
            <div class="meta-row">
        ...
        <div class="main-grid">
          ...
          <div class="col-right card">
            <div class="card-header"><h3>💬 讨论区</h3></div>
            <div class="chat-window" ref="chatWindow">
              <div v-if="comments.length === 0" class="empty-text">还没有人发言</div>
  ...
  onMounted(async () => {
    ...
    if (projectId) await fetchProjectData(projectId)
    else { errorMsg.value = '无效的企划ID'; loading.value = false }
  })
  ...
  const fetchProjectData = async (id) => {
    ...
    // 1. 查询企划 (去掉 profiles 联表)
    const { data: proj, error } = await supabase.from('items').select('*').eq('id', id).single()
    
    if (error) {
      console.error("Project Fetch Error:", error)
      // 如果是 406 Not Acceptable，通常也是查不到数据
      errorMsg.value = error.code === 'PGRST116' ? '找不到该企划' : error.message
      loading.value = false
      return
    }
    ...
    // 2. 加载子模块 (分开加载，一个失败不影响其他)
    try {
      const tRes = await supabase.from('project_timeline').select('*').eq('project_id', id).order('event_date', { ascending: true })
      if(tRes.data) timeline.value = tRes.data
  
      const taskRes = await supabase.from('project_tasks').select('*').eq('project_id', id).order('created_at', { ascending: true })
      if(taskRes.data) tasks.value = taskRes.data
  
      const cRes = await supabase.from('project_comments').select('*').eq('project_id', id).order('created_at', { ascending: true })
      if(cRes.data) comments.value = cRes.data
      
      scrollToBottom()
    } catch (e) { console.warn("Sub module load failed", e) }
  ...
  const submitTask = async () => {
    ...
    if (!error) { newTaskInput.value = ''; fetchProjectData(project.value.id) }
  }
  ...
  const deleteTask = async (tid) => {
    if (confirm('删除?')) await supabase.from('project_tasks').delete().eq('id', tid).then(() => fetchProjectData(project.value.id))
  }
  const addTimelineEvent = async () => {
    ...
    fetchProjectData(project.value.id)
  }
  ...
  const deleteTimeline = async (tid) => {
    if (confirm('删除?')) await supabase.from('project_timeline').delete().eq('id', tid).then(() => fetchProjectData(project.value.id))
  }
  const sendComment = async () => {
    if (!currentUser.value) return alert('请登录')
    if (!newComment.value) return
    await supabase.from('project_comments').insert([{ project_id: project.value.id, user_id: currentUser.value.id, content: newComment.value }])
    newComment.value = ''; fetchProjectData(project.value.id)
  }
  ...
  .error-box { text-align: center; padding: 50px; background: #fff5f5; border: 1px solid #ffcdd2; border-radius: 8px; color: #c62828; margin-top: 50px; }
